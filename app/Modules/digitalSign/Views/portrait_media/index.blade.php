                                            ...
                                            <td>
                                                @if ($db_col === 'media')
                                                    @if ($r->media_type === 'image')
                                                        <img src="{{ asset('storage/' . $r->media) }}"
                                                             alt="Media"
                                                             style="height: 60px; object-fit: cover; cursor: zoom-in;"
                                                             class="img-thumbnail"
                                                             onclick="showZoomImage('{{ asset('storage/' . $r->media) }}')">
                                                    @elseif ($r->media_type === 'video')
                                                        <video height="80" controls>
                                                            <source src="{{ asset('storage/' . $r->media) }}" type="video/mp4">
                                                            Your browser does not support the video tag.
                                                        </video>
                                                    @else
                                                        <span class="text-muted"><i>Tidak diketahui</i></span>
                                                    @endif
                                                @elseif ($db_col === 'media_type')
                                                    <span class="badge bg-secondary">{{ ucfirst($r->media_type) }}</span>
                                                @elseif ($db_col === 'durasi')
                                                    @php
                                                        $dur = (int) $r->{$db_col};
                                                        $jam = floor($dur / 3600);
                                                        $menit = floor(($dur % 3600) / 60);
                                                        $detik = $dur % 60;

                                                        $formatDurasi = '';
                                                        if ($jam > 0) $formatDurasi .= $jam . ' jam ';
                                                        if ($menit > 0) $formatDurasi .= $menit . ' menit ';
                                                        if ($detik > 0 || $formatDurasi === '') $formatDurasi .= $detik . ' detik';
                                                    @endphp
                                                    {{ $formatDurasi }}
                                                @elseif (in_array($db_col, $boolean_column))
  ...
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content bg-transparent border-0 shadow-none">
      <div class="modal-body text-center p-0">
        <img src="" id="zoomedImage" class="img-fluid rounded shadow" alt="Zoomed Image" style="max-height: 90vh;">
      </div>
    </div>
  </div>
</div>
